    ...
    **Parâmetros não obrigatórios**:
    ...
    def barcode(self):
        ...
        num = "%s%1s%04d%010d%24s" % (self.codigo_banco,
                                      self.moeda,
                                      due_date_days,
                                      Decimal(self.valor_documento.replace('.','').replace(',','.')) * 100,
                                      self.campo_livre)
    ...
    def _set_valor_documento(self, val):
        ...
        if type(val) is Decimal:
            ...
            if valor.endswith('.00'):
                self._valor_documento = Decimal(str(val).replace(',','.'))
            elif valor[-3] == '.':
                self._valor_documento = Decimal(str(val).replace(',',''))
            else:
                self._valor_documento = Decimal(str(val).replace('.','').replace(',','.'))

            #print('Valor Parametro', val)
            #print('Valor ', valor)
            #print('Valor Propriedade', self._valor_documento)
