...
from app.forms import GeneralModelConverter
from app.widgets import FormWidget, ShowWidget, ListWidget, SearchWidget, ListHeaderWidget
from app.actions import ActionItem
...
class BaseView(object):
        ...
        if self.base_permissions is None:
            ...
            for attr_name in dir(self):
                if hasattr(getattr(self, attr_name), '_permission_name'):
                    ...
                    self.base_permissions.add('' + permission_name)
    ...
    def create_blueprint(self, appbuilder,
        ...
        if not static_folder:
            ...
            #print("******************************************************",self.endpoint, self.template_folder, self.route_base)
        else:
            self.blueprint = Blueprint(self.endpoint, __name__,
                                       url_prefix=self.route_base,
                                       template_folder=self.template_folder,
...
class BaseFormView(BaseView):
    ...
    """
    form_template = 'frontend/general/model/edit.html'
...
class BaseCRUDView(BaseModelView):
    ...
    list_template = 'frontend/general/model/list.html'
    """ Your own add jinja2 template for list """
    edit_template = 'frontend/general/model/edit.html'
    """Your own add jinja2 template for edit"""
    add_template = 'frontend/general/model/add.html'
    """ Your own add jinja2 template for add """
    show_template = 'frontend/general/model/show.html'
    ...
    def _init_titles(self):
        ...
        if not self.list_title:
            self.list_title = 'Lista ' + self._prettify_name(class_name)
        if not self.add_title:
            self.add_title = 'Inclusão ' + self._prettify_name(class_name)
        if not self.edit_title:
            self.edit_title = 'Alteração ' + self._prettify_name(class_name)
        if not self.show_title:
            self.show_title = 'Visualização ' + self._prettify_name(class_name)
