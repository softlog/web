...
from app.flask_sqlalchemy_tny.model import Model
from app.flask_sqlalchemy_tny._compat import itervalues, string_types, xrange
from app.flask_sqlalchemy_tny.model import DefaultMeta
from app.flask_sqlalchemy_tny import utils
...
class _EngineConnector(object):
    ...
    def get_uri(self):
        if self._bind is None:
            ...
            #uri = session.get('uri')
            try:
                #uri = SecureCookieSession['uri']
                #uri = request.cookies.get('uri')
                
                
                #try:
                    #print("********ESTA E A PILHA***************")
                    #print(connection_stack.top.__dict__)
                #except Exception as e:
                #    pass 
                #try:
                #    print('Chamada de ',request.path)
                #except:
                #    print('Request indefinido')
                #Use database's uri in mode setup for all dababases multi tenancy 
                uri = self._app.config.get('URI_SETUP')
                origem = ""
                #Use database's uri in mode login multi tenancy
                if uri is None:                                       
                    uri = getattr(connection_stack.top,'uri_login',None)
                    origem = "Login"
                    #print('uri_login', uri)

                #Use database's uri in mode request multi tenancy
                if uri is None:
                    uri = getattr(connection_stack.top,'uri_request',None)
                    #print('uri_request', uri)

                #Use database's uri in mode request multi tenancy
                if uri is None:
                    uri = getattr(connection_stack.top,'uri_api_softlog',None)
                    #print('uri_api_softlog', uri)

                #Use database's uri in mode standard 
                if uri is None:
                    uri = self._app.config['SQLALCHEMY_DATABASE_URI']
                    #print('uri_padrao', uri)

                print('Conectando com a uri', uri)   
                ...
                #print("***************************")
                #print('Neste momento não posso me conectar', e.args[0])
                #print("***************************")
                uri = self._app.config['SQLALCHEMY_DATABASE_URI']
