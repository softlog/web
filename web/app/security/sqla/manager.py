...
from .models import User, Permission, PermissionView, RegisterUser, ViewMenu, Role, StringConexoes
from app.models.gerais import Cidades, Filial, Empresa
...
class SecurityManager(BaseSecurityManager):
    ...
    stringconexoes_model = StringConexoes
    cidade_model  = Cidades
    filial_model  = Filial
    #user_filial_model = assoc_user_filial
    #user_role_model = assoc_user_role
    ...
    def __init__(self, appbuilder):
        ...
        super(SecurityManager, self).__init__(appbuilder)
        #self.create_db()
        #if self.auth_type == c.AUTH_DB_TNY:
        #    self.create_db_tny()
    ...
    def create_db_tny(self):
        uri_tmp = self.appbuilder.app.config['SQLALCHEMY_DATABASE_URI']
        dbs = self.get_all_db_tny()
        for db in dbs:
            #print("PROCESSANDO *******************",db.db_uri_pg())
            self.appbuilder.app.config['SQLALCHEMY_DATABASE_URI'] = db.db_uri_pg()
            self.appbuilder.session.close()
            self.appbuilder.session.remove()
            self.create_db()
        
        self.appbuilder.app.config['SQLALCHEMY_DATABASE_URI'] = uri_tmp 
    ...
    def find_user(self, username=None, email=None, cod_empresa=None, cod_filial=None):
        """
            Finds user by username or email
        """
        if cod_empresa:
            return self.get_session.query(self.user_model,self.filial_model).\
                                    join(self.user_filial_model).join(self.filial_model).\
                                    filter(and_(func.lower(self.user_model.username) == func.lower(username),
                                                self.filial_model.codigo_empresa == cod_empresa,
                                                self.filial_model.codigo_filial == cod_filial)).first()
        if username:
            role_admin = self.appbuilder.app.config.get('AUTH_ROLE_ADMIN')            
            return self.get_session.query(self.user_model).\
                                                join(self.user_role_model).join(self.role_model).\
                                                filter(and_(func.lower(self.user_model.username) == func.lower(username),
                                                            func.lower(self.role_model.name) == func.lower(role_admin))).first()
                                                            
        elif email:
            return self.get_session.query(self.user_model).filter_by(email=email).first()


    def find_db_tny(self, id_db_tny=None, alias=None):
        """
            Finds database tenancy by id or alias
        """
        if id_db_tny:
            return self.get_session.query(self.stringconexoes_model).filter(self.stringconexoes_model.id_string_conexao==id_db_tny).first()
        elif alias:
            return self.get_session.query(self.stringconexoes_model).filter_by(banco_dados=alias).first()

    def find_filial(self, cod_empresa=None, cod_filial=None):
        """
            Generic function find a filial by it's id empresa and filial
        """
        return self.get_session.query(self.filial_model).filter(and_(self.filial_model.codigo_empresa==cod_empresa,
                                                               self.filial_model.codigo_filial==cod_filial)).first()
    ...
    def get_all_db_tny(self):
        return self.get_session.query(self.stringconexoes_model).filter(and_(self.stringconexoes_model.softlog_web==1, 
                                                                             self.stringconexoes_model.id_string_conexao != 1)).all()
    ...
    def get_user_by_tny(self, id_user_tny):        
        partes = id_user_tny.split("@")        
        if len(partes)==1:            
            return (self.get_session.query(self.user_model).filter(func.lower(self.user_model.id_user_db_tny) == func.lower(id_user_tny)).first(),None)
        else:            
            cod_empresa = partes[1].split('-')[0]
            cod_filial =  partes[1].split('-')[1]            
            return self.get_session.query(self.user_model,self.filial_model).\
                                join(self.user_filial_model).join(self.filial_model).\
                                filter(and_(func.lower(self.user_model.id_user_db_tny) == func.lower(partes[0]),
                                                            self.filial_model.codigo_empresa == cod_empresa,
                                                            self.filial_model.codigo_filial == cod_filial)).first()                                
    """
        ----------------------------------------
