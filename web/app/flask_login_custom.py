...
class LoginManager(object):
    ...
    def __init__(self, app=None, add_context_processor=True):
        ...
        self.anonymous_filial = AnonymousFilialMixin
    ...
    def _load_from_cookie(self, cookie):
        if self.token_callback:
            ...
            if user is not None:                
                ...
                getattr(connection_stack.top,'user',None)
                setattr(connection_stack.top,'user',user)
        ...
        if getattr(connection_stack.top,'user',None) is not None:
            app = current_app._get_current_object()
            user_loaded_from_cookie.send(app, user=_get_user()[0])
            user_loaded_from_cookie.send(app, filial=_get_user()[1])
    ...
    def _load_from_header(self, header):
        ...
        if user is not None:
            ...
            app = current_app._get_current_object()
            user_loaded_from_header.send(app, user=_get_user()[0])
            user_loaded_from_header.send(app, filial=_get_user()[1])
        else:
            self.reload_user()
    ...
    def _load_from_request(self, request):
        ...
        if user is not None:
            self.reload_user(user=user)
            app = current_app._get_current_object()
            user_loaded_from_request.send(app, user=_get_user()[0])
            user_loaded_from_request.send(app, filial=_get_user()[1])
        else:
            self.reload_user()
...
class UserMixin(object):
    ...
    def get_id_tny(self):
        try:
            return [unicode(self.id_user_tny),None]
        except AttributeError:
            raise NotImplementedError('No `id` attribute - override `get_id_tny`')
...
class FilialMixin(object):
    '''
    This is the default object for representing an anonymous Filial.
    '''
    id_filial = None 

    def get_filial_description(self):
        return False
...
class AnonymousFilialMixin(object):
    '''
    This is the default object for representing an anonymous Filial.
    '''
    id_filial = None 
...
def login_user(user, remember=False, force=False, filial=None):
    '''
    ...
    if filial is None:
        ...
        session['user_id'] = user_id + '@' + filial.codigo_empresa + '-' + filial.codigo_filial
    ...
    getattr(connection_stack.top,'filial',None)
    setattr(connection_stack.top,'filial',filial)

    user_logged_in.send(current_app._get_current_object(), user=_get_user()[0])
    user_logged_in.send(current_app._get_current_object(), filial=_get_user()[1])
...
def logout_user():
    ...
    if user is not None and not user.is_anonymous():
        ...
        user_logged_out.send(current_app._get_current_object(), filial=filial)
...
def _user_context_processor():
    return dict(current_user=_get_user()[0],current_filial=_get_user()[1])
